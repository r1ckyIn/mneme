...
| V-07 | (auxiliary) | No `learn-os` references survive in non-skip-listed files (audit log files in `.planning/state/` and historical commits exempt) | T-RNM-01 incomplete-rename | shell-grep | `! rg -i 'learn[-_ ]?os' --hidden --no-ignore --glob '!.git/' --glob '!.planning/state/' --glob '!.planning/phases/00-identity-branding-lock/' \| read` | ❌ W0 | ⬜ pending |
| V-08 | (auxiliary) | `.learnos/rules/` directory convention has been replaced with `.<finalname>/rules/` | T-RNM-02 lockstep-miss | shell-grep | `! rg -F '.learnos/rules/' --hidden --no-ignore --glob '!.git/' --glob '!.planning/state/' --glob '!.planning/phases/00-identity-branding-lock/' \| read` | ❌ W0 | ⬜ pending |
