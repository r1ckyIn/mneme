...
4. **Round 5 prototype handoff alignment (added 2026-05-09 — 12 deltas A-04 through A-15)** — see `01-AMENDMENT-2026-05-09.md`. Headlines: **A-04** $-cost meter / `~/.mneme/usage.jsonl` / daily cap stay deleted (Round 4 closure propagated to ROADMAP success #4 and to plan 01-06). **A-05** 5 regions get Notion-style 6-dot drag handle (Phase 1 visual + no-op; Phase 3 wires logic). **A-06** chat-footer vault-context button = NotebookLM↔Claude reply-mode toggle (Phase 1 visual + Phase 2 spawn-arg wire). **A-07** left pane = Finder-style table shell (empty body; headers + breadcrumb + 6-dot handle ship). **A-08** Cmd+. added to unbound list (10 combos total); Stop only via Send/Stop button click. **A-09** new Ctx + Session usage meter replaces deleted cost meter (reads `result.usage.input_tokens`, rolling session timer; `.cost.warning` red repurposed for ctx ≥ 90%). **A-10** titlebar meta surface ships: connection indicator + vault-path text + settings cog. **A-11** settings cog opens placeholder modal "Settings wires in Phase 2". **A-12** middle column splits into video (top, Phase 6 wire) + preview (bottom, Phase 4 wire) via 4px row splitter (50/50, non-resizable in Phase 1). **A-13** chat footer 1:1 Claude Code layout: left = Auto-mode dropdown + + + 🎙️ mic + chevron + vault-ctx; right = model pill `Opus 4.7 1M · Max` + Send/Stop; mic + Auto-mode = visual placeholder, voice = REQ-19 v1.x; spawn-args do NOT pass `--model`. **A-14** tool-use card `<details>`-collapsible: open during streaming, closed on `result` event (matches old Claude web UX). **A-15** prototype `Mneme.html` is the visual-fidelity SSOT for Phase 1; planner / executor compares against it when UI-SPEC.md is silent on visual treatment.
