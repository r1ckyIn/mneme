...
承担物理边界双向：
- **mneme-side（项目内）**：dev-only Svelte forwarder + Tauri dev commands + npm-script bridge（`gsd-dev-screenshot` / `gsd-dev-snapshot` / `gsd-dev-scan-logs`）
- **GSD upstream（`~/.claude/get-shit-done/`）**：verify-work workflow 三处 patch + 8 个 `verify.*` SDK handlers + 共享 `templates/visual-review.html`
- **Cross-cutting workflow rules**：execute-phase 静默 + 4-bucket 强制 + Visual Review HTML 一次性 / gitignored
