...
- [ ] **Phase 2: Vault + Canvas/Ed Sync + Onboarding** - Local-first markdown vault, Canvas/Ed import + incremental sync, sync status surface, settings UI, first-run wizard (15/15 plans executed; verify pending — 12 shipped 2026-05-16; 3 gap-closure plans shipped 2026-05-17 close 4 BLOCKERs + 6 WARNINGs)
...
**Plans**: 12 shipped + 3 gap-closure staged 2026-05-17 (Wave 0-8 ship structure; gap-closure cohort waves 1-2; 5 TDD plans for Rust modules + state singletons, 7 execute plans for IPC + Svelte UI + integration). Locked 2026-05-16 via `/gsd-plan-phase 2 --tdd --research`; checker iteration 2 PASSED.
Plans:
**Wave 1**

- [x] 02-01-PLAN.md — Wave 0 dependency adds + 24 failing-stub tests + onDragDropEvent spike (probe + Errata) + T-2-01/T-2-02 HIGH-severity path traversal/symlink tests
- [x] 02-02-PLAN.md — Wave 1 TDD `vault_writer.rs` (WriteContext + canonicalize-parent + chmod 644→write→444 + Drop guard) + 4 placeholder module stubs
- [x] 02-06-PLAN.md — Wave 1 TDD `import-state.svelte.ts` + `vault-state.svelte.ts` + pure `derivePillState`

**Wave 2** *(blocked on Wave 1 completion)*

- [x] 02-03-PLAN.md — Wave 2 TDD `config.rs` + `onboarding.rs` (atomic temp+rename siblings)
- [x] 02-04-PLAN.md — Wave 2 TDD `vault_index.rs` (rusqlite WAL + CRUD + reconcile + folder-batch helper)

**Wave 3** *(blocked on Wave 2 completion)*

- [x] 02-05-PLAN.md — Wave 3 TDD `import_controller.rs` (tokio task + CancellationToken + per-file emit)

**Wave 4** *(blocked on Wave 3 completion)*

- [x] 02-07-PLAN.md — Wave 4 Tauri IPC wiring (13 commands + dragDropEnabled + plugin-dialog + capability gates + vault_move safe-copy)

**Wave 5** *(blocked on Wave 4 completion)*

- [x] 02-08-PLAN.md — Wave 5 Onboarding route + +layout redirect + Onboarding.svelte + Step1 + Step4 + StepRail

**Wave 6** *(blocked on Wave 5 completion)*

- [x] 02-09-PLAN.md — Wave 6 Step2AuthCheck + Step3VaultPicker + Step5AddCourse + Step6DemoImport (Browse-only, no drag listener — BLK-3) + claude_auth_check command

**Wave 7** *(blocked on Wave 6 completion)*

- [x] 02-10-PLAN.md — Wave 7 SettingsPanel + 4 category bodies + Cmd+, shortcut + categories/move tests

**Wave 8** *(blocked on Wave 7 completion)*

- [x] 02-11-PLAN.md — Wave 7 ImportDialog + DuplicateResolutionDialog shell (D-08 deferred backend) + StatusPill + HistoryModal + DropzoneOverlay + ReconciliationOverlay (spinner-only counter, D-14 DR1)
- [x] 02-12-PLAN.md — Wave 8 TitlebarMeta wiring + +page.svelte mounts (DropzoneOverlay gated by `$page.route.id !== '/onboarding/[step]'`) + Cmd+I + interaction-paradigm thread Cmd+I row

**Gap-closure wave (shipped 2026-05-17 via `/gsd-execute-phase 2 --gaps-only`):**

- [x] 02-13-PLAN.md — Gap-closure Wave 1 TDD: backend data safety (CR-02 `_inbox` clash + CR-03 canon paths + WR-01 env_broken); 4 tasks, parallel with 02-15 (commits `c5f464d` RED → `42a0ddc` pin → `12c52da` GREEN → `ba5379c` cites → `0431a27` SUMMARY)
- [x] 02-15-PLAN.md — Gap-closure Wave 1: hardening cluster (WR-02 RelockGuard + WR-03 Onboarding race + WR-06 audit regex + WR-11 empty vaultRoot); 5 tasks, parallel with 02-13 (commits `008ec3c` → `ace379e` → `ca91383` → `94c8113` → `51ae7e9` → `0be65f8` SUMMARY, merged via `b35ad8a`)
- [x] 02-14-PLAN.md — Gap-closure Wave 2 (after 02-13): reconciliation contract + UI fixes (CR-01 Browse/Move split + CR-04 reconcile:progress emit + WR-08 layout race); 5 tasks (commits `cad6bd9` RED → `b824e91` callback → `7505e6c` emit → `6323ac3` Browse split → `d049b15` race fix → `b739525` cites → `2e810f5` SUMMARY)

**Plans:** 15/15 executed (12 shipped 2026-05-16 + 3 gap-closure shipped 2026-05-17; verify pending — closes 4 BLOCKERs + 6 of 11 WARNINGs)
**UI hint**: yes

### Phase 02.1: UI fixes from dogfood and audit — 3 BLOCKER + 8 WARNING + 3 INFO (INSERTED)

**Goal:** Close 3 BLOCKER + 8 WARNING + 3 INFO findings from post-Phase-2-ship dogfood + UI audit (B1 DropzoneOverlay invisible / B2 token violation / B3 keychain auth blocker / W1-W8 polish + a11y / I1-I3 polish). Restore dogfood-ready UX before Phase 02 ships.
**Requirements**: REQ-03, REQ-06, REQ-13, REQ-14, REQ-16 (inherited from parent Phase 02 scope)
**Depends on:** Phase 2
**Plans:** 13/13 plans complete

Plans:
- [x] 02.1-01-PLAN.md — Wave 0 RED tests for B3 + W6 + W5 (inspector_keychain_detect.rs + course_code_validation.rs + import-error-fallback.test.ts)
- [x] 02.1-02-PLAN.md — B3 GREEN: claude_auth_check subprocess probe + capability surface + audit gate 9
- [x] 02.1-03-PLAN.md — B2: VaultCategory move-complete token swap (--color-success → --color-warm-dark-soft)
- [x] 02.1-04-PLAN.md — W6 GREEN: vault_writer.validate_course_code replaces regex; SECURITY.md audit section
- [x] 02.1-05-PLAN.md — W5 GREEN: import-error.ts raw-error tail enrichment in fallback branch
- [x] 02.1-06-PLAN.md — B1: DropzoneOverlay dark veil + hero card per D-04 / Mneme 3 SSOT
- [x] 02.1-07-PLAN.md — W1: ImportStatusPill icons + spinner SVG + state-specific backgrounds
- [x] 02.1-08-PLAN.md — W2: Onboarding step fly transition + src/lib/motion.ts prefersReducedMotion helper + unit test
- [x] 02.1-09-PLAN.md — W3: SettingsPanel rail-item :focus-visible orange ring
- [x] 02.1-10-PLAN.md — W4: Step3VaultPicker "Choose another folder" ghost-link
- [x] 02.1-11-PLAN.md — W7: keymap deferral marker across 5 Phase 02 doc files (Cmd+,/I/P/R → v1.x deferred per D-06)
- [x] 02.1-12-PLAN.md — W8: TitlebarMeta remove blanket no-drag CSS; selective per-child carve-out
- [x] 02.1-13-PLAN.md — I2 + I3 polish; I1 formally deferred to Phase 3 (backlog seed)
...
| 2. Vault + Canvas/Ed Sync + Onboarding | 12/15 | Gap-closure | shipped 2026-05-16; gap-closure plans staged 2026-05-17 |
